...
int main(void){
    ...
    *h_deaths = 7;
    ...
    // Abrindo arquivo de saída e escrevendo resultados
    FILE *fileOutput = fopen("resultado.txt", "w");
    if(fileOutput == NULL){
        printf("Erro ao abrir o arquivo de saída\n");
        return 1;
    }

    fprintf(fileOutput, "Numéro de mortes: %d\n", *h_deaths);
    fprintf(fileOutput, "Numéro de sobreviventes: %d\n", *h_survivors);
    fclose(fileOutput);
